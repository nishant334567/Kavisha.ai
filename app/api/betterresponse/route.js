...
export async function POST(req) {
  const { userMessage, sessionId, brand, prompt, historyTillNow } =
    await req.json();
  ...
  try {
    ...
    if (results.matches && results.matches.length > 0) {
      ...
      console.log(
        `Found ${results.matches.length} matches, ${relevantMatches.length} above threshold`
      );
