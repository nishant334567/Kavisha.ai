...
export default function AssessmentDetailsForm({
  ...
  const inputClass =
    "w-full rounded-xl border border-border bg-input px-4 py-2 text-foreground font-fredoka focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/30";

  return (
    <div className="rounded-xl border border-border bg-card p-8 text-foreground shadow-lg">
      <h2 className="mb-6 text-2xl font-semibold text-highlight font-fredoka">
        Quiz/Survey Details
      </h2>
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center pt-4">
          ...
          <button
            onClick={onNext}
            className={`rounded-full bg-highlight px-6 py-3 font-medium text-white transition-colors hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-ring/30 font-fredoka ${onCancel ? "ml-auto" : ""}`}
          >
            Next: Add Questions
          </button>
