  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex justify-between items-center p-3 sm:p-6 border-b border-[#004A4E]/20 bg-[#004A4E]/5">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-[#004A4E] rounded-full flex items-center justify-center text-white font-semibold text-xs sm:text-sm flex-shrink-0">
              {(selectedSessionLogs.user?.name || "U").charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-base sm:text-lg font-medium text-[#004A4E] truncate">
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#004A4E]/10 transition-colors flex-shrink-0 ml-2"
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 bg-[#004A4E]/[0.03]">
          {1 === 2 ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex flex-col items-center gap-4">
                <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#004A4E]"></div>
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-3 ${log.role === "user"
                      ...
                      : "bg-[#004A4E]/10 text-gray-800 border border-[#004A4E]/20"
                      }`}
                  >
                    ...
                    {log.role === "assistant" && (
                      <>
                        {Array.isArray(log.sourceUrls) && log.sourceUrls.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-[#004A4E]/15">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {log.sourceUrls.map((url) => (
                                <a
                                  ...
                                  className="text-xs px-2 py-1 rounded-md bg-[#004A4E]/5 hover:bg-[#004A4E]/10 text-[#004A4E] underline truncate max-w-[180px] sm:max-w-[280px]"
                                  title={url}
                                >
                                  {url}
                                </a>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {log.sourceChunkIds.map((id) => (
                                <button
                                  ...
                                  className="text-xs px-2 py-1 rounded-md bg-[#004A4E]/10 hover:bg-[#004A4E]/20 text-[#004A4E] truncate max-w-[180px] sm:max-w-[240px]"
                                  title={id}
                                >
                                  {id}
                                </button>
        ...
        <div className="p-3 sm:p-4 bg-white border-t border-[#004A4E]/20 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 text-xs sm:text-sm text-gray-600">
          <div>
